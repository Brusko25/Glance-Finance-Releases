...
Widgets request quotes every 15 seconds and chart history every five minutes, or immediately after a range change. CoinGecko quotes are shared and cached for two minutes; its month history is cached for five minutes and filtered for 1D / 1W / 1M (usually hourly samples). Requests are spaced out to respect public API limits, so a newly added coin can take a few seconds to load. Rate limits and other failures show an unavailable/stale state and retry automatically, waiting longer after each failure (30 seconds up to 5 minutes); Refresh now retries at once. CoinGecko history does not provide per-candle volume bars. The panel refreshes prices every minute while it's on screen, and at once when you open it. Widgets and the panel pause scheduled refreshes while Windows is locked or asleep. Requests run asynchronously; selection changes cancel older widget requests. News is loaded from Yahoo's RSS feed and can be refreshed on Overview. Headlines link to their publishers.
